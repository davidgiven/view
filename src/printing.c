...
static uint8_t expand_line(void)
{
    ...
    uint8_t a = get_current_fmt_cmd_byte(&y);
    //     clc
    //     beq return_37
    if (a == 0)
        return x;
...
static void dm_fmt_cmd(void)
{
    ...
    uint8_t a = macro_executing_flag;
    if (a != 0)
        return;
    //     bne return_42
    //     lda last_macro_ptr
...
c9788:
    ...
    x_1++;
    //     cmp l97b0,x
    if (a_2 == l97b0_data[x_1])
        goto c9788;
    //     lda l97b0,x
    uint8_t a_3 = l97b0_data[x_1];
    if ((int8_t)a_3 < 0)
        goto c97ae;
    //     cmp #0x20 ; ' '
    //     bcc return_46
    ...
    uint16_t* result, uint8_t* y, uint8_t x)
{
    ...
    {
        //     jsr get_current_fmt_cmd_byte
        uint8_t a_1 = get_current_fmt_cmd_byte(y);
        //     beq c9821
        if (a_1 == 0)
        {
        ...
        if (a_1 == 0x7c)
        {
            ...
            uint8_t a_2 = get_next_fmt_cmd_byte(y);
            //     beq c9821
            if (a_2 == 0)
                goto c9821;
            //     iny
...
static void print_loop(addr_t ptr5)
{
    ...
    while (1)
    {
        ...
        addr_t cursor = prepare_output_line(ptr5, &ptr3);
        if (cursor == 0)
            return;
        //     jsr sub_c916a
    ...
    c8fb9_l:
        //     lda macro_executing_flag
        uint8_t a_4 = macro_executing_flag;
    ...
    c8fce_l:
        ...
        uint8_t y_4 = l0080;
        //     lda print_flags
        uint8_t a_7 = print_flags;
        if (((int8_t)a_7 < 0))
        {
            uint8_t a_8 = microspacing_flag;

            if (a_8 != 0)
            {
                microspace_word_processor(&y_4);
                continue;
            }
        }
        do
        {
            uint8_t a_9 = ram[cursor + y_4];
            y_4++;
            a_10 = convert_char_for_printing(a_9, &x, &is_tab);
            print_char_x_times(a_10, x);
        } while (a_10 != 0x0d);
        //     inc register_value_l
        //     bne c8ffb
        //     inc register_value_l+1
...
c8c95:
    ...
    uint8_t a_1 = get_byte_from_file();
    //     beq c8cf2
    if (a_1 == 0)
    {
        eof_1 = true;
...
static void render_header_or_footer(uint8_t* text)
{
    ...
    uint8_t x = copy_header_footer_text(section_start_1);
    //     txa
    uint8_t a_2 = x;
    //     beq c9355
    if (a_2 == 0)
...
c9355:
    ...
    //     jsr sub_c93c8
    uint8_t x_2 = copy_header_footer_text(section_start_2);
    //     jsr sub_c93be
    uint8_t a_6 = get_right_margin();
    //     beq c937b
    //     stx l0081
    //     sec
    //     sbc l0081
...
c92d4:
    ...
    uint8_t x = top_margin;
    //     jsr print_vertical_space
    print_vertical_space(x);
    //     lda headers_enabled_flag
    uint8_t a_4 = headers_enabled_flag;
    //     beq c92e8
    if (a_4 != 0)
    {
        // (the 6502 passed the buffer address in YX; the C passes it directly)
...
static void start_microspacing_if_active(uint8_t a)
{
    ...
    uint8_t x = print_flags;
    //     bpl return_25
    if (!(x & 0x80))
        return;
    //     ldx microspacing_flag
    uint8_t x_1 = microspacing_flag;
    //     bne c9177
    if (x_1 == 0)
        return;
    // c9177:
...
c9188_normal_entry:
    ...
    if (read_limit != 0)
    {
        addr_t cursor_1 = read_limit;
        current_format_line_ptr = cursor_1;
        return cursor_1;
    }
    // c91a3:
c91a3:
...
c91a7:
    ...
    {
        //     lda ((*macro_cursor)),y
        uint8_t a_1 = ram[(*macro_cursor) + y];
        //     cmp #4
        //     beq c9184
        if (a_1 == 4)
        {
        ...
        for (;;)
        {
            ...
            ram[RAM_CURRENT_LINE_BUF + x] = a_1;
            //     inx
            x++;
            //     cmp #0x0d
            //     beq c91c2
...
c91c2:
    ...
    // c91d0:
    addr_t cursor_2 = ptr1;
    current_format_line_ptr = cursor_2;
    //     clc
    // return_26:
    //     rts
    return cursor_2;
...
c91da:
    ...
    y++;
    //     lda ((*macro_cursor)),y
    uint8_t a_3 = ram[(*macro_cursor) + y];
    //     sec
    //     sbc #0x30 ; '0'
    //     bcc c9225
...
c91f5:
    ...
    y_1++;
    //     lda (ptr5),y
    uint8_t a_5 = ram[read_limit + y_1];
    //     cmp #0x0d
    //     beq c9223
    if (a_5 == 0x0d)
        goto c9223;
    //     jsr sub_c9228
    {
        enum parse_register_result_t r = parse_register_reference(a_5);
        //     beq c91f5
...
c9209:
    ...
    y_1++;
    //     lda (ptr5),y
    uint8_t a_6 = ram[read_limit + y_1];
    //     cmp #0x0d
    //     beq c9223
    if (a_6 == 0x0d)
        goto c9223;
    //     jsr sub_c9228
    {
        enum parse_register_result_t r_1 = parse_register_reference(a_6);
        //     beq c9209
...
c921b:
    //     sta current_line_buffer,x
    ram[RAM_CURRENT_LINE_BUF + x] = a_6;
    //     inx
    x++;
    //     cpx #0x82
    //     bcc c9209
...
static read_block_status_t read_next_output_line(addr_t limit, addr_t* cursor)
{
    ...
    uint8_t a = printing_from_file_flag;
    //     beq c9260
    if (a == 0)
    {
...
static void compute_lines_remaining_on_page(void)
{
    ...
    //     sbc header_margin
    //     bcc c930d
    if (a_1 < header_margin)
        goto c930d;
    a_1 -= header_margin;
    //     clc
    //     sbc bottom_margin
    //     bcc c930d
    if (a_1 < bottom_margin)
        goto c930d;
    a_1 -= bottom_margin;
    //     sbc footer_margin
    //     bcc c930d
    if (a_1 < footer_margin)
        goto c930d;
    a_1 -= footer_margin;
    x = a_1;
c930d:
...
static uint8_t copy_header_footer_text(uint8_t* text)
{
    ...
    c93f2:
        //     lda (((uint8_t*)&text)[0]),y
        uint8_t a_3 = text[y_1];
        if ((int8_t)a_3 < 0)
            goto c93e6;
        //     iny
        y_1++;
        //     jsr render_register
        render_register(a_3, x);
        //     jmp c93ce
    }
}
...
static void output_left_margin(void)
{
    ...
    bool parity = get_page_parity();
    //     lda left_margin
    uint8_t a = left_margin;
    //     bcc c9415
    if (parity)
    {
        uint8_t x = two_sided_flag;
