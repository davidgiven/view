...
c968f:
    ...
    flags &= ~FLAG_C;
    //     adc last_macro_ptr
    adc(&flags, (uint8_t)(last_macro_ptr & 0xff));
    //     sta last_macro_ptr
...
c96a2:
    ...
    flags |= FLAG_C;
    //     sbc last_macro_ptr
    sbc(&flags, (uint8_t)(last_macro_ptr & 0xff));
    //     tax
    x = a;
    set_flags(&flags, x);
    //     lda himem+1
    ...
    sbc(&flags, (uint8_t)((last_macro_ptr >> 8) & 0xff));
    //     bne c96b8
    if (!(flags & FLAG_Z))
        goto c96b8;
    //     cpx #0x97
...
c8fe6_inline:
    ...
    set_flags(&flags, ram[RAM_REGISTER_VALUE_L + 1]);
c8ffb_inline:
    //     ldx line_spacing
    x = line_spacing;
    //     lda l0021
...
c91c2:
    ...
    //     inc ptr3+1
    ptr3 =
        (ptr3 & 0x00ff) | ((uint16_t)((uint8_t)((ptr3 >> 8) & 0xff) + 1) << 8);
    // c91cc:
c91cc:
    //     lda ptr1
    a = (uint8_t)(ptr1 & 0xff);
...
static void sub_c92f0(void)
{
    ...
    flags &= ~FLAG_C;
    //     sbc top_margin
    sbc(&flags, top_margin);
    //     bcc c930d
    if (!(flags & FLAG_C))
        goto c930d;
    //     sbc header_margin
    sbc(&flags, header_margin);
    //     bcc c930d
    if (!(flags & FLAG_C))
        goto c930d;
    //     clc
    flags &= ~FLAG_C;
    //     sbc bottom_margin
    sbc(&flags, bottom_margin);
    //     bcc c930d
    if (!(flags & FLAG_C))
        goto c930d;
    //     sbc footer_margin
    sbc(&flags, footer_margin);
    //     bcc c930d
    if (!(flags & FLAG_C))
        goto c930d;
    //     tax
