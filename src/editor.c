...
static uint8_t find_marker_at_position(uint8_t y, uint8_t* ptr);
static void unpack_line(uint8_t* ptr);
static void update_markers_to_format_buffer(void);
void check_for_embedded_ruler(uint8_t* ptr);
...
static void unpack_line_into_buffer(uint8_t* ptr);
...
void split_line_at_wrap(uint8_t* ptr);
...
static void insert_line_at_cursor(uint8_t* ptr);
...
static void cf6_split_line_key(void)
{
    ...
    uint8_t a_1 = current_format_line_ptr[y_1];
    ...
    uint32_t sum = (uint32_t)(current_line_ptr - &ram[0]) + x;
    if (sum > 0xffff)
    {
        f6_insert_line_key();
        return;
...
static void cf8_mark_as_ruler_key(void)
{
    ...
    uint8_t a_1 = 0x2e;
    //     sta (current_format_line_ptr),y
    current_format_line_ptr[y] = a_1;
    //     iny
    y++;
    //     lda #0x2e ; '.'
    uint8_t a_2 = 0x2e;
    //     sta (current_format_line_ptr),y
    current_format_line_ptr[y] = a_2;
    //     inc l0074
    line_counter++;
    //     lda l006e
    //     bmi c9f5f
...
static void delete_key(void)
{
    ...
    xpos--;
    //     ldy xpos
    //     lda (current_edit_line_ptr),xpos
    uint8_t a_1 = ram[RAM_EDIT_BUFFER + xpos];
    //     pha
    {
...
static void f3_delete_to_eol_key(void)
{
    ...
    delete_edit_buffer_bytes_at_xpos(a);
    return;
}
...
void return_key(void)
{
    ...
    //  find_next_line))
    uint16_t sum = (current_line_ptr - &ram[0]) + y;
    //     jsr sub_c9de1
...
static void sf1_swap_case_key(void)
{
    ...
    //     ldy xpos
    //     lda (current_edit_line_ptr),xpos
    uint8_t a = ram[RAM_EDIT_BUFFER + xpos];
    //     jsr is_uppercase
    //     bcs f13_right_key
    if (!isalpha(a))
    {
    ...
    //     eor #0x20 ; ' '
    a ^= 0x20;
    //     sta (current_edit_line_ptr),xpos
...
edit_command_loop:
    ...
    uint8_t y = scratch_offset;
    //     iny
    y++;
    //     sty input_buffer_offset+1
    scratch_offset = y;
    //     sta (ptr1),y
    // (16-bit arithmetic: ptr1[y], with the carry across the low-byte
...
static void delete_edit_buffer_bytes_at_xpos(uint8_t x)
{
    ...
    do
    {
        ...
        if (!(x == 0x0c))
        {
            ...
            //     adc #0
            //     bne cae93
            //     (fall through to cae91 if the high byte is 0, which never
            //     happens
            ...
            uint16_t marker_val =
                (y >= temp_save) ? RAM_EDIT_BUFFER + (y - scratch_offset) : 0;
            markers_array[x / 2] = marker_val ? &ram[marker_val] : NULL;
            //     jmp cae78
            continue;
        }
        // cae98:
...
c9c56:
    //     ldy l0039
    uint8_t y_4 = column_position;
    //     cpy l003a
    if (y_4 < ruler_buffer_len)
    {
        //     lda (current_ruler_ptr),y
        uint8_t a_4 = current_ruler_ptr[y_4];
        //     and #0xdf
        a_4 &= 0xdf;
        //     cmp #0x42 ; 'B'
        if (a_4 == 0x42)
...
bool adjust_margins_at_left_margin(void)
{
    ...
    //     and #0x81
    a &= 0x81;
    if (!(a != 0 || find_left_margin_stop() >= 0))
    {
        //     jsr get_line_length
        uint8_t line_len = get_line_length();
        //     lda xpos
        ...
        else
        {
            ...
            if (!(y >= screen_column))
            {
                ...
                //     sbc ruler_left_stop
                //     bcc caf2a
                // (sbc with C=1 in is a plain subtraction; if it borrows, skip
                // the store.
                //  Otherwise the carry into the following adc is 1: a += xpos +
                //  1)
                if (screen_column < ruler_left_stop)
                    goto caf2a;
                a_3 = screen_column - ruler_left_stop + xpos + 1;
            }
            //     adc xpos
            // caf28:
            //     sta xpos
    ...
    caf2a:
        ...
        if (!insert_byte_at_xpos(y))
            return true;
        // bcs return_79 — insertion failed
        //     inc l0074
        line_counter++;
    }
    // caf31:
...
bool insert_edit_buffer_bytes_at_xpos(uint8_t x)
{
    ...
    //     adc input_buffer_offset+1
    //     bcs cae03
    // (adc with C=0 is xpos plain addition; carry means overflow)
    a_1 += scratch_offset;
    if (a_1 < scratch_offset)
    {
        beep();
        return false;
    }
...
cae27:
    ...
    //     cpy xpos
    //     bne cae27
    // (byte shift consolidated into xpos single memmove)
cae52:
    if (y != xpos)
        goto cae27;
    int copy_len = MAX_LINE_LENGTH - (int)xpos - (int)scratch_offset;
...
void set_marker_to_here(uint8_t x)
{
    ...
    if (len >= xpos)
    {
        ...
        //     ldy #0
        //     lda (current_format_line_ptr),y
        uint8_t a = current_format_line_ptr[0];
        //     ldy xpos
    ...
    markers_array[x] = &ram[marker_addr];
    //     rts
}
...
uint8_t* adjust_pointers(uint8_t* insert_ptr, ptrdiff_t size_delta)
{
    ...
    ca9db:
        if (!(x >= ARRAY_SIZE(markers_array)))
        {
            ...
            // (carry-in to the first sbc is always 1 (all three paths to ca9e7
            // leave
            //  C=1), so this is a plain 16-bit subtraction: pointer_array[x] -=
            //  tmp67)
        }
        else
        ca9e7:
        ...
        ca9f1:
            ...
            x++;
    } while (x != sizeof(pointer_array) / sizeof(uint8_t*));
    // loop_ca9f7: (6416)
    //     ldy #0 (6417)
    // loop_ca9f9: (6418)
...
c8b91:
    ...
    // c8bb7:
    //     jsr sub_c8c5f
    screen_column = upper_case_unless_folding(a_2);
    //     sta l0083
    do
    {
        // c8bbc:
...
static uint8_t get_line_length(void)
{
    ...
    //     ldy #0
    //     lda (current_format_line_ptr),y
    uint8_t a = *current_format_line_ptr;
    //     jsr check_for_command_prefix
    command_prefix_t cp = check_for_command_prefix(a);
    //     php
    //     ldy #0x84
...
c9871:
    ...
    y_2++;
    //     cpy l0046
    if (y_2 >= justify_gap_count)
        y_2 = 1;
    //     dey
    y_2--;
    //     ldx l0046
    uint8_t x_3 = justify_gap_count;
    do
    {
        // c98b5:
        //     lda l0045
        ...
        //     lda l0082
        uint8_t a_8 = screen_row;
        //     sec
        //     sbc input_buffer,y
        // (sec makes this a plain 8-bit subtraction; the result's Z flag is
        // tested
    ...
    do
    {
        ...
        if (!(a_12 != 0x20))
        {
            //     bne c9920
            //     lda l0081
            uint8_t a_13 = scratch_index;
            if (!(a_13 == 0))
            {
                //     sty l0084
                temp_save = y_5;
                //     ldy l0039
                uint8_t y_6 = column_position;
                //     cpy l0046
                //     lda #0
                //     bcs c9912
                if (y_6 < justify_gap_count)
...
static void recalculate_cursor_xpos(void)
{
    ...
    if (!(a != 0))
    {
        ...
        do
        {
            ...
            a += column_position;
        } while (a >= column_position);
    }
    // ca624:
    //     lda #0
    a = 0;
    //     sta l0079
    line_change_pending_flag = a;
    //     tay                                                               ;
    //     Y=0x00
    uint8_t y_1 = a;
    // loop_ca629:
...
void redraw_editor(void)
{
    ...
    {
        //     lda l0073 (5214)
        uint8_t a_3 = display_start_row;
        //     ora l006f (5215)
        a_3 |= scroll_repeat_count;
        //     bne ca28e (5216)
        if (a_3 != 0)
            goto ca28e;
        // ca28e: (5219)
        //     lda current_line_ptr+1 (5220) cmp l0012 (5221) bcc ca29c (5222)
        //     bne ca2dc (5223) lda current_line_ptr (5224) cmp l0011 (5225) bcs
        //     ca2dc (5226)
    }
    else
    ca28e:
    ...
    ca29c:
        ...
        //     ldy l0012 (5232) lda l0011 (5233) cpy top+1 (5234) bcc ca2b2
        //     (5235) bne ca30d (5236) cmp top (5237) bcs ca30d (5238)
    ...
    ca30d:
        ...
        line_lengths[x_4] = screen_maxcolumn;
        //     lda l0033 (5316)
        //     sta ruler_stack_ptr (5317)
        ruler_index_ptr = saved_ruler_index_scroll;
        //     ldy l0012 / lda l0011  (these loaded bytes are clobbered by
        //     advance_to_next_line's scan; y is left holding the offset past
        ...
        screen_scrollup();
        //     ldx #0 (5330) ldy screen_height (5331) jsr set_cursor_position
        //     (5332)
    ...
    {
        //     lda hscroll_pos / clc / adc screen_width / sbc #3 / cmp l0072
        //     (5357)
...
static void append_to_output_buffer(uint8_t a)
{
    ...
    if (editor_output_pos >= MAX_LINE_LENGTH)
        return;
    //     sta output_buffer,l0048
    output_buffer[editor_output_pos] = a;
    //     inc l0048
    editor_output_pos++;
    // return_13:
    //     rts
...
c998a:
    ...
    uint8_t a_3 = ruler_right_stop - ruler_left_stop + 2;
    //     sta input_buffer_offset+1
    scratch_offset = a_3;
    ...
    // was l0038 in 6502, now local to format_paragraph
    //     sty l0047
    format_src_index = y_1;
    //     sty l0039
    column_position = y_1;
    //     sty l0038
    soft_hyphen_flag = y_1;
    //     sty l0046
    justify_gap_count = y_1;
    //     sty bottom_margin
    bottom_margin = y_1;
...
c99b6:
    //     sty l0048
    editor_output_pos = y_1;
    //     ldy l0047
    uint8_t y_2 = format_src_index;
    // loop_c99ba:
    do
    {
        //     jsr sub_ca536
...
c99c9:
    ...
    {
        // c99c7:
        // PROVISIONAL: Character processing loop. Reads one byte from the
        // source document line. PROVISIONAL: l0047 tracks the read position,
        // l0048 tracks the write position. PROVISIONAL: Handles tabs (0x09),
        // soft hyphens / break markers (0x1a), soft spaces (0x0b), CR (0x0d),
        // PROVISIONAL: and ordinary characters.
        //  c99c9:
        //     lda (current_line_ptr),y
        a_5 = current_line_ptr[y_2];
        //     iny
        y_2++;
        //     sty l0047
        format_src_index = y_2;
        if (!(a_5 != 9))
        {
            //     bne c99e0
            //     jsr sub_ca5ae
        ...
        {
            //     bne c99ee
            // PROVISIONAL: Soft hyphen / break marker (0x1a): if l0046
            // (word-start flag) is non-zero, PROVISIONAL: skip the marker and
            // continue reading. Otherwise, treat it as a word-break:
            // PROVISIONAL: set l0046 (word-break state), output a space, and go
            // to c9a2e.
            //  c99e4:
            //     lda l0046
            ...
            goto c9a2e;
            // c99ee:
            // PROVISIONAL: Soft space (0x0b, margin tab). If
            // input_buffer_offset is non-zero (already PROVISIONAL: in a
            // word-break), treat as break marker (c99e4). If l0038 (soft-hyphen
            // flag) PROVISIONAL: is set, also treat as break. Otherwise set
            // l0038 and advance the line width PROVISIONAL: (l0080) by
            // ruler_left_stop, or snap column to ruler_left_stop if below it.
        c99ee:
            //     cmp #0x0b
            if (a_5 != 0x0b)
                goto c9a11;
...
static bool find_next_word_boundary(uint8_t y)
{
    ...
    do
    {
        ...
        //     beq c9b2f
        //     cmp #0x0d
        if (cp != NO_COMMAND_PREFIX || a_1 == 0x0d)
            goto c9b2f;
        if (!(y != 0))
        {
            //     sty l0084
        ...
        c9aef:
            ...
            if (a_3 == 0x0b)
            {
                ...
                //     sec
                //     ror l0084
                // (The 6502 sets bit 7 by rolling left, setting C, and rolling
                // right
...
static uint8_t compute_display_start_line(void)
{
    ...
    //  `line` holds it: after l0034 successful find_previous_line it is
    //  advanced to the previous line (left in tmp01), and on failure/early-exit
    //  it is the starting line.  In all cases the top-of-screen line is the
    //  final value of `line` — i.e. tmp01 after l0034 success, or the starting
    //  line on failure/early-exit.)
    uint8_t* line = current_line_ptr;
    // loop_ca465:
    for (;;)
    {
...
static void unpack_line(uint8_t* ptr)
{
    ...
    {
        //     lda (current_line_ptr),y
        uint8_t a2 = current_line_ptr[y_1];
        //     cmp #0x0d
        if (a2 == 0x0d)
            break;
        //     beq caac8
        //     sta (current_format_line_ptr),y
        current_format_line_ptr[y_1] = a2;
        //     iny
        y_1++;
        //     bne loop_caabd
    } while (y_1 != 0);
...
static void update_markers_to_format_buffer(void)
{
    ...
    do
    {
        ...
        if (!(idx == 0x0c))
        {
            ...
            //     adc #0
            //     sta markers_array+1,x
            // (16-bit arithmetic: markers_array[idx] = current_format_line_ptr
            // + y;
            //  the bne caad5 loops while the high byte of the result is
            //  non-zero)
            {
                uint16_t val = (current_format_line_ptr - &ram[0]) + y;
...
void check_for_embedded_ruler(uint8_t* ptr)
{
    ...
    //     jsr sub_cab6e
    //     bne cac4c
    // (inlined: Z = (*ptr == RULER_BYTE))
    if (*ptr == RULER_BYTE)
        push_onto_ruler_index(ptr);
    //     pla
    //     tay
    //     pla
    //     rts
...
static bool write_line_back_to_document(void)
{
    ...
    if (!(edit_buffer_unpacked_flag == 0))
    {
        ...
        screen_column = get_line_length();
        //     lda l003b
        uint8_t a_1 = edit_line_len;
        //     sec
        //     sbc l0083
        //     bcc ca8df
        //     beq ca8ed
        // (sbc with C=1 is edit_buffer_unpacked_flag plain subtraction;
        // edit_buffer_unpacked_flag holds edit_buffer_unpacked_flag - l0083 on
        // all
        //  paths, and the borrow/zero flags select the branch)
        {
            ...
            if (minuend < screen_column)
                goto ca8df;
            // borrow: edit_buffer_unpacked_flag < l0083
            if (a_1 == 0)
                goto ca8ed;
        }
        //     sta ((uint8_t*)&tmp67)[0]
    ...
    ca8ed:
        ...
        do
        {
            ...
            {
                a_6 = current_format_line_ptr[y_1];
                //     cmp #0x10
                if (a_6 == 0x10)
                    a_6 = 0x20;
            }
