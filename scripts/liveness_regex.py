...
def line_defs_uses(stripped, index):
    ...
    for helper in INLINE_HELPERS:
        ...
        for bit in FLAG_BITS:
            # flags &= ~FLAG_C (clc) / flags |= FLAG_C (sec) are unconditional
            # writes — the old flag value is overwritten, not read.
            pat_clr = r'flags\s*&=\s*~FLAG_' + bit
            pat_set = r'flags\s*\|=\s*FLAG_' + bit
            if re.search(pat_clr, stripped) or re.search(pat_set, stripped):
                defs.add('flags:' + bit)
            else:
        ...
        # Direct register assignments (including declarations)
...
def get_local_info(lines, start, end, callee_live_out=None):
    ...
    local_live_in = set()
    defined_so_far = set()
    local_decls = set()   # variables declared with uint8_t inside the function
    call_sites = []
    for i in range(start, end):
        ...
        # A return statement ends a control-flow path.  Reset defined_so_far
        # so that post-return uses on a different branch are treated as live-in,
        # but keep local declarations in scope (they are valid on all paths).
        if stripped == 'return;':
...
def analyze_files(files):
    ...
    while changed and fp_iter < 10:
            ...
            for i in range(end - 1, start - 1, -1):
                ...
                if stripped == 'return;':
                    ...
                    if callee in ALL_IN_OUT:
                        ...
                        if callee in ('create_default_ruler', 'move_cursor_to_top_of_document', 'clear_cmd', 'cb05a'):
                            print(f"    backward (initialise): {callee} call, live_before={{{format_vars(live)}}}")
        ...
        for name in all_funcs:
            ...
            if name == 'create_default_ruler':
                print(f"  Iter {fp_iter}: create_default_ruler: required={{{format_vars(required)}}}, all_defs={{{format_vars(all_defs)}}}")
